...
import { ICategory, ICategoryResponse, IUser, IService, IServiceResponse,IUserResponse } from "@/interfaces";
import { useBookServiceMutation, useGetBookingsQuery } from "@/lib/redux/features/apis/bookings_api";
...
const BookingsPage: FC<pageProps> = ({}) => {
  ...
  const handleReBooking = useCallback(async (payload:{
    amount:number,
    location: string,
    serviceId: string,
  }) => {
  ...
  }, [makeBooking, bookingResponse.isError, bookingResponse.isSuccess, bookingResponse.data?.data, router]);
//get valild bookings
                ...
                >
                  ...
                  {booking.status === 'in progress' && (
                    <div className="flex justify-center mt-7 md:mt-0 sm:justify-start items-center gap-4 lg:max-w-[25%] w-full">
                      ...
                      }} variant={"primary"} className="px-5 text-xs">
                        Chat
                      </Button>
                  ...
                  {booking.status === 'pending' && (
                    <div className="flex justify-center mt-7 md:mt-0 sm:justify-start items-center gap-4 lg:max-w-[25%] w-full">
                      ...
                      <Button onClick={()=> handlePayment(booking._id)} variant={"skyButton"} className="text-xs">
                        Proceed Payment
                      </Button>
                    </div>
                  )}
                  {booking.status === 'cancelled' && (
                    <div className="flex justify-center mt-7 md:mt-0 sm:justify-start items-center gap-4 lg:max-w-[25%] w-full">
                      <Button onClick={()=> handleReBooking({amount:booking.amount,location: booking.location, serviceId: booking.serviceId._id})} variant={"primary"} className="px-5 text-xs">
                        Re Book
                      </Button>
                      {/* <Button variant={"skyButton"} className="text-xs">
                        Reschedule
